...
callHandler :: (CallProv TaskQ par1, CallProv TaskQ par2, Show par1, Show par2) => 
                  CallRes par2 -> Text -> Int -> STrans (ContT Bool) TaskQ NoSplitter '[CallIdle TaskQ par1 "call1"] _ _ ()
callHandler callRes2 dest timeoutSec = do
  ...
  try @("call1" :? IsCallAlive) $ do
    ...
    invoke #timerCalling (StartTimer timeoutSec)
    try @("call2" :? IsCallAlive) $ do
      handleEvents $ do
        ...
        on @("timerCalling" :? IsTimerTriggered) $ do
          _t1 :: _ <- whatNext
